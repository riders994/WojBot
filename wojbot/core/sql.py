...
Connection details come from ``sql_config.yml`` at the repo root (``conn_uri``),
which is gitignored.
"""
...
SQL_CONFIG_PATH = PROJECT_ROOT / "sql_config.yml"
SQL_DIR = PROJECT_ROOT / "sql"
DEFAULT_CONNECTION = "wojbot"


def load_conn_uri(path: Path = SQL_CONFIG_PATH) -> str:
    """Read the ``conn_uri`` from the SQL config file."""
    try:
        with open(path, encoding="utf-8") as f:
            cfg = yaml.safe_load(f) or {}
    except FileNotFoundError as exc:
        raise ConfigError(
            f"{path.name} not found. Copy sql_config.example.yml to {path.name} "
            "and set conn_uri."
        ) from exc
    conn_uri = cfg.get("conn_uri")
    if not conn_uri:
        raise ConfigError(f"'conn_uri' is missing or empty in {path.name}.")
    return conn_uri
...
class SqlService:
    ...
    async def connect(self, config_path: Path = SQL_CONFIG_PATH) -> None:
        """Open (or replace) the default connection from ``sql_config.yml``."""
        conn_uri = load_conn_uri(config_path)
