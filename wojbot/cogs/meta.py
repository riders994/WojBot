...
TOPICS: dict[str, tuple[str, str]] = {
    ...
    "rumors": (
        ...
        "play in more than one, you're asked which.\n\n"
        ...
        "*league release* only from the Commissioner, which is Admin-tier. A "
    ...
    "permissions": (
        "Permission tiers",
        "**The bot owner passes every check, everywhere. Server administrators "
        "pass both role tiers in their own server.**\n\n"
        "**Commissioner** — `commissioner_roles`, default `Commish`. Gates "
        "`/commish load|sync|publish|dump`, all of `/commish db`, and "
        "`/commish season add|platform|current`.\n\n"
        "**Admin** — `admin_roles`, default `mods`, `Champion`, `Commish`. Gates "
        "`/setup`, `/verify add|remove`, the read-only `/commish` commands, and "
        "the Commissioner source in `/rumor`.\n\n"
        "**Bot owner** — not configurable. Gates `/sql` and `/setup global`.\n\n"
        "Roles are matched **by name**, per server, so renaming a role in "
        "Discord silently unconfigures it. `/verify show` tells you where you "
        "stand and flags configured names no live role matches. Change them with "
        "`/verify add` and `/verify remove`."
    ),
}
